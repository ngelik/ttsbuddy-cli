...
export TTSBUDDY_API_URL="${TTSBUDDY_API_URL:-https://www.ttsbuddy.com/v1/hn-cli-demo}"
export TTSBUDDY_API_KEY="${TTSBUDDY_API_KEY:-ttsb_demo_hn}"

if ! command -v ttsbuddy >/dev/null 2>&1; then
  echo "ttsbuddy was not found on PATH."
  echo "In Codespaces this should be built by the devcontainer."
  echo "Locally, run: go build -o ./ttsbuddy . && PATH=\"$ROOT_DIR:$PATH\" ./demo/hn-demo.sh"
  exit 1
fi

mkdir -p out

echo "== TTSBuddy CLI HN demo =="
...
- It is for no-signup evaluation on Hacker News.
